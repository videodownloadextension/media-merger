<img width="979" height="900" alt="image" src="https://github.com/user-attachments/assets/88aae011-0242-4fdf-8fbf-9de6ae777550" />


## 📋 功能特点：
- 自动扫描文件夹中的视频和音频文件
- 将相同文件名的文件配对合并（如：video.mp4 + video.mp3）
- 合并后的文件保存到原文件夹下的 merged 目录
- 如果 merged 目录中已有同名文件，将自动跳过
- 基于文件大小的准确进度显示（MB/秒，剩余时间）
- 自动记住上次选择的文件夹
- 内置 ffmpeg，无需单独安装
- 便携式设计，配置文件保存在程序所在目录
- 支持中途停止处理
- 完美兼容 Windows 7/8/10/11

## 🎬 支持格式：
视频格式：MP4, AVI, MKV, MOV, WMV, FLV, WEBM, M4V
音频格式：MP3, WAV, AAC, FLAC, M4A, OGG, WMA

## 📖 使用步骤：
1. 点击「浏览」按钮选择包含音视频文件的文件夹
2. 点击「开始合并」按钮开始处理
3. 查看实时进度（文件大小/速度/剩余时间）
4. 可随时点击「停止」按钮取消处理

## 各平台下载

| 平台 | 文件名 | 说明 |
|------|--------|------|
| Windows | `音视频合并工具_Windows.exe` | 双击运行 |
| Linux | `音视频合并工具_Linux` | 需要执行权限 |
| macOS | `音视频合并工具_macOS` | 需要执行权限 |

## 使用方法

### Windows
直接双击 `音视频合并工具_Windows.exe`

### Linux
```bash
chmod +x 音视频合并工具_Linux
./音视频合并工具_Linux
```

### macOS

```bash
chmod +x 音视频合并工具_macOS
./音视频合并工具_macOS
```

## ⚠️ 注意事项：
- 确保视频和音频文件名相同（不包括扩展名）
- 本程序已内置 ffmpeg，无需额外安装
- 配置文件保存在程序所在目录的 config 文件夹中
- 处理过程中请勿关闭程序或删除源文件
- 进度基于文件大小估算，最终文件可能略有误差

